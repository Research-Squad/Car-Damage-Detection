...
class PathType:
    def __init__(self, exists=True, val_type: Union[Callable[[str], bool], str, None] = 'file', dash_ok=True):
    ...
    def __call__(self, value):
        if value == '-':
            ...
            if self._val_type == 'dir':
                raise ArgumentError('standard input/output (-) not allowed as directory path')
            elif self._val_type == 'symlink':
                raise ArgumentError('standard input/output (-) not allowed as symlink path')
            ...
            if self._exists:
                ...
                if not os.path.isdir(parent_dir):
                    raise ArgumentError("parent path is not a directory: '%s'" % parent_dir)
                elif not os.path.exists(parent_dir):
                    raise ArgumentError("parent directory does not exist: '%s'" % parent_dir)
